...
impl Scope {
    ...
    pub fn add_id(&mut self, id: Rc<Node>) -> Result<String, SemanticErrors> {
        match id.get_kind() {
            KindIdentifier::Function | KindIdentifier::ForwardFunction => {
                try!(self.add_function(id.clone()));
            }
            _ => {
                try!(self.add_var(id.clone()));
            }
        };
    ...
    pub fn add_var(&mut self, id: Rc<Node>) -> Result<String, SemanticErrors> {
        match self.vars.get(&id.get_name()) {
            Some(_res) => {
                return Err(SemanticErrors::DuplicateIdentifier {
                    name: id.get_name(),
                })
            }
            None => {}
                        ...
                        match id.get_kind() {
                            ...
                            _ => {
                                return Err(SemanticErrors::OtherError {
                                    msg: "Ошибка при перегрузке функции"
                                        .to_string(),
                                })
                            }
                        };
...
fn create_system_scope() -> Box<Scope> {
    ...
    let tboolean = Rc::new(EnumType::new(
        var_list.clone(),
        value_map.clone(),
        "boolean".to_string(),
        0,
    )) as Rc<Type>;
    let tfalse = Rc::new(EnumType::new(
        var_list.clone(),
        value_map.clone(),
        "boolean".to_string(),
        0,
    )) as Rc<Type>;
    let ttrue = Rc::new(EnumType::new(
        var_list.clone(),
        value_map.clone(),
        "boolean".to_string(),
        1,
...
impl SemanticChecker {
    pub fn new() -> SemanticChecker {
        SemanticChecker {
            system_scope: create_system_scope(),
            scopes: vec![Box::new(Scope::new())],
        }
    }
    pub fn add_scope(&mut self) {
        self.scopes.push(Box::new(Scope::new()));
    }
        ...
        if scope.count_forward > 0 {
            for (name, overrides) in scope.functions {
                for (sign, over) in &overrides {
                    match over.get_kind() {
                        KindIdentifier::ForwardFunction => {
                            return Err(SemanticErrors::ErrorInForwardDecl {
                                name: name.to_string(),
                                sign: sign.to_string(),
                            })
                        }
    ...
    pub fn get_id(&self, name: String) -> Option<Rc<Node>> {
        ...
        match self.system_scope.get_overrides(&name) {
            Some(res) => for (_sign, func) in res {
    ...
    pub fn create_function(
        &mut self,
        id: Rc<DeclFunctionNode>,
        shift: i32,
    ) -> Result<Rc<Node>, SemanticErrors> {
