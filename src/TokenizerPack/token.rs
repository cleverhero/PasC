...
impl Token {
    ...
    pub fn new(
        ...
        if token_type_str == "id" {
            ...
            if text.to_lowercase() == "integer" {
                token_type_str = "integer_type".to_string();
            }
            if text.to_lowercase() == "double" {
                token_type_str = "double_type".to_string();
            }
            if text.to_lowercase() == "char" {
                token_type_str = "char_type".to_string();
            }
        }

        if token_type_str == "string" {
            value = Value::Str {
                v: text[1..text.len() - 1].to_string(),
            };
        }
