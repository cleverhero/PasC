        ...
        while state != "end" {
            match self.reader.next_char() {
                FatChar::Char { ch } => {
                    ...
                    if state == "none" {
                        let error = ErrorState::Critical {
                            err: TokenizerErrors::SimpleError {
                                x: self.pointer.y,
                                y: self.pointer.x,
                            },
                        };
        ...
        loop {
            match self.next_token() {
                ...
                Err(error) => match error {
                    ErrorState::Critical { err } => {
                        return Err(CompilerErrors::TokenizerError { err });
                    }
