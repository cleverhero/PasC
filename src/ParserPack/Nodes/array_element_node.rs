...
impl ArrayElementNode {
	pub fn new(parent: Rc< Node >, index: Rc< Node >) -> Result< ArrayElementNode, SemanticErrors > {
		let self_type = try!( parent.get_type().unwrap().get_by_index(index.get_type().unwrap()) );
		Ok( ArrayElementNode { parent, index, self_type } )
	}
}
...
impl PrintableNode for ArrayElementNode {
	fn get_children(&self) -> Vec< &PrintableNode > { vec![self.index.as_printable(), self.parent.as_printable()] }
...
impl Node for ArrayElementNode {
	...
	fn get_name(&self) -> String { self.parent.get_name() + "[" + &self.index.get_type().unwrap().value_as_str() + "]" }
